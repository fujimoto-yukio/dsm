...
Java1.5以来のプログラム製造で、随分と進化したJavaと初めてのGUIアプリ開発、JavaFXに苦戦しながら、（やっつけで[^1]）DSMアプリのプロトタイプを作りました。<br>
出来上がったアプリは、DSMをCSVから読み込み、要素を上下（左右）に移動、並び替えるだけの簡単なものでしたが、DSMの動きが直接見えて画期的なものに思えました。

[^1]: ネットで漁ったサンプルコードをコピペしたところがたくさんあります。初めて見たJavaのラムダ式は呪文にしか見えませんでした（笑
